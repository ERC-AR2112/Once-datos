...
encig1 <- read_csv("Encig/Data/encig2025_01_sec1_A_3_4_5_8_9_10.csv")
encig2 <- read_csv("Encig/Data/encig2025_02_residentes_sec_2.csv")
encig3 <- read_csv("Encig/Data/encig2025_03_sec_6.csv")
encig4 <- read_csv("Encig/Data/encig2025_04_sec_7.csv")
encig5 <- read_csv("Encig/Data/encig2025_05_sec_8.csv")
encig_11 <- read_csv("Encig/Data/encig2025_01_sec_11.csv")
...
### Cómo reciben las ciudades sedes el mundial

encig1 %>% count(CVE_AREAM)


encig1 %>% count(P4_1A) #Agua
encig1 %>% count(P4_3A) #Alumbrado público
encig1 %>% count(P4_5A) #Recolección de basura
encig1 %>% count(P4_6A) #Policia
encig1 %>% count(P4_7A) #Calles y avenidas


encig1 %>% count(P5_6A) #Servicio de salud estatal
encig1 %>% count(P5_8A) #Servicio de luz
...
corr_larga <- encig1 %>%
  select(UPM, EST_DIS, FAC_P18, CVE_ENT, starts_with("P3_3_", ignore.case = TRUE)) %>% 
  pivot_longer(
    cols = starts_with("P3_3_", ignore.case = TRUE),
    names_to = "Institucion_cod",
    values_to = "Respuesta"
  ) %>%
  mutate(Institucion_cod = toupper(Institucion_cod)) %>% 
  ...
  mutate(
    Categoria = case_when(
      Respuesta == 1 ~ "1. Muy frecuente",
      Respuesta == 2 ~ "2. Frecuente",
      Respuesta == 3 ~ "3. Poco frecuente",
      Respuesta == 4 ~ "4. Nunca",
      Respuesta == 9 ~ "9. Ns/Nc",
      TRUE ~ NA_character_
    ),
    Institucion = case_when(
      Institucion_cod == "P3_3_01"  ~ "Universidades Públicas",
      Institucion_cod == "P3_3_02"  ~ "Policías",
      Institucion_cod == "P3_3_03"  ~ "Hospitales públicos",
      Institucion_cod == "P3_3_04"  ~ "Presidencia de la República",
      Institucion_cod == "P3_3_05"  ~ "Empresarios",
      Institucion_cod == "P3_3_06"  ~ "Gobernaturas de los estados",
      Institucion_cod == "P3_3_07"  ~ "Compañeros (as) de trabajo",
      Institucion_cod == "P3_3_08"  ~ "Presidentes municipales",
      Institucion_cod == "P3_3_09"  ~ "Familiares (primos, tios, etc.",
      Institucion_cod == "P3_3_10" ~ "Sindicatos",
      Institucion_cod == "P3_3_11" ~ "Vecinos (as)",
      Institucion_cod == "P3_3_12" ~ "Cámaras de Diputados y Senadores",
      Institucion_cod == "P3_3_13" ~ "Medios de comunicación",
      Institucion_cod == "P3_3_14" ~ "Institutos electorales",
      Institucion_cod == "P3_3_15" ~ "Comisiones de Derechos Humanos",
      Institucion_cod == "P3_3_16" ~ "Escuelas Públicas a nivel básico",
      Institucion_cod == "P3_3_17" ~ "Jueces (ezas) y Magistrados (as)",
      Institucion_cod == "P3_3_18" ~ "Instituciones religiosas",
      Institucion_cod == "P3_3_19" ~ "Partidos Políticos",
      Institucion_cod == "P3_3_20" ~ "Guardia Nacional",
      Institucion_cod == "P3_3_21" ~ "Ejercito y marina",
      Institucion_cod == "P3_3_22" ~ "Ministerio público",
      Institucion_cod == "P3_3_23" ~ "ONGs",
      Institucion_cod == "P3_3_24" ~ "Organismos Públicos Autónomos")
  ) %>%
  filter(!is.na(Categoria))

## Estableciendo el diseño muestral
encig_design <- corr_larga %>%
  as_survey_design(
    ids = UPM,
    strata = EST_DIS,
    weights = FAC_P18,
...
##Preparacxión de las gráficas
orden_instituciones <- corr_res %>%
  filter(Categoria %in% c("1. Muy frecuente", "2. Frecuente")) %>%
  group_by(Institucion) %>%
  summarise(corrupcion= sum(proporcion)) %>%
  arrange(corrupcion) %>%
  pull(Institucion)
...
corr_res %>% 
  filter(Institucion %in% c("Gobernaturas de los estados", "Presidencia de la República", "Presidentes municipales")) |> 
  ggplot(aes(x = proporcion, y = Institucion, fill = Categoria)) +
  geom_col(position = "stack", width = 0.8) +
  scale_fill_manual(
    values = c(
      "1. Muy frecuente" = "#1a9641",
      "2. Frecuente" = "#a6d96a",
      "3. Poco frecuente" = "#fdae61",
      "4. Nunca" = "#d7191c",
      "9. Ns/Nc" = "#d9d9d9"
    )
  ) +
  scale_x_continuous(labels = percent_format(accuracy = 1)) +
  labs(
    title = "Nivel de corrupción percibida en los diferentes niveles de gobierno",
    subtitle = "Proporción de la población urbana de 18 años y más",
    x = "Porcentaje poblacional",
    y = NULL,
    fill = NULL,
...
### Mapa, confianza en los gobernadorxs
corr_estatal <- encig_design %>%
  filter(Institucion == "Gobernaturas de los estados") %>%
  mutate(
    corrupcion = if_else(
      Categoria %in% c("1. Muy frecuente", "2. Frecuente"),
      1, 0
    )
  ) %>%
  group_by(CVE_ENT) %>%
  summarise(
    prop_corrupcion = survey_mean(corrupcion, na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(CVE_ENT= str_pad(CVE_ENT, width = 2, side = "left", pad = "0"))
...
##Preparando el marco geoestadistico
mapa_mexico <- st_read("C:/Users/USER/OneDrive/Documentos/Marco_geografico2025/Integrado/conjunto_de_datos/00ent.shp")

mapa_corr<- mapa_mexico %>%
  left_join(corr_estatal, by = "CVE_ENT")


ggplot(mapa_corr) +
  # aes(fill) mapea nuestra variable calculada al color de relleno del estado
  geom_sf(aes(fill = prop_corrupcion), color = "white", size = 0.3) +
  ...
  labs(
    title = "Percepción de corrupción en el Gobierno Estatal por Entidad Federativa",
    subtitle = "Proporción de población de 18 años",
    caption = "Fuente: Elaboración propia con datos de ENCIG 2025 (INEGI)."
  ) +
